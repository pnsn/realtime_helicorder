...
const QUAKE_START_OFFSET = Duration.fromObject({hours: 1});
...
function main() {
  ...
  if ( state.endTime) {
    ...
    state.endTime = "now";
    chooserEnd = DateTime.utc();
  }
  const chooserStart = chooserEnd.minus(Duration.fromISO(state.duration));
  ...
  dateChooser.updateCallback = time => {
    ...
    throttleRedisplay = window.setTimeout(() => {
      let updatedTime = time.plus(Duration.fromISO(state.duration));
...
function createEmptySavedData(config) {
  const luxDur = Duration.fromISO(config.duration);
  ...
  if (DateTime.isDateTime(end)) {
    ...
    plotEnd = DateTime.utc().endOf('day').plus(ONE_MILLISECOND);
  } else {
    plotEnd = DateTime.fromISO(config.endTime).toUTC();
  }
  let timeRange = Interval.before(plotEnd, luxDur);
  let hash = {
    config: config,
    timeRange: timeRange,
...
function doPlotHeli(config) {
  ...
  .then(netArray => {
    ...
    let rawDataList = [];
    hash.chanTR
      .filter(sdd => sdd.startTime < DateTime.utc())
      .forEach(sdd => {
    ...
    if (! minMaxSeismogram) {
      ...
      let nowMarker = { markertype: 'predicted', name: "now", time: DateTime.utc() };
...
`
    ...
    markers.push({ markertype: 'predicted', name: "now", time: DateTime.utc() });
    if (hash.seisData) {
  ...
  if (hash.seisData) {
    ...
    hash.heli.addEventListener("heliclick", hEvent => {
      ...
      if ( ! Number.isNaN(Number.parseFloat(hwValue))) {
        // assume seconds
        dur = Duration.fromMillis(1000*Number.parseFloat(hwValue));
      } else {
        dur = Duration.fromISO(hwValue);
      }
      if (dur.toMillis() > 0 ) {
        hash.halfWidth = Duration.fromMillis(dur.toMillis()/2);
      }
      if (hash.station) {
        drawSeismograph(hash);
      } else {
...
function drawSeismograph(hash) {
  ...
  if (! halfWidth) { halfWidth = Duration.fromISO("PT5M"); }
  const seismographDisp = seismographDiv.querySelector("sp-organized-display");
  const interval = Interval.fromDateTimes(hash.centerTime.minus(halfWidth), hash.centerTime.plus(halfWidth));
  const overlapQuakes = hash.traveltimes.filter(tt => {
    ...
    const quakeInterval = Interval.after(tt.quake.time, { seconds: lastArrival.time});
...
function updateDateChooser(config) {
  let dateChooser = document.querySelector("sp-datetime");
  if ( config.endTime && config.duration) {
    dateChooser.updateTime(sp.util.isoToDateTime(config.endTime).minus(Duration.fromISO(config.duration)));
  } else {
    ...
    dispElements.forEach(orgDispItem => {
      if (orgDispItem.plottype.startsWith(sp.organizeddisplay.SEISMOGRAPH)) {
        ...
        seismograph.seisData.forEach(sdd => {
          ...
          sdd.timeRange = Interval.fromDateTimes(start, end);
        });
      }
    });
